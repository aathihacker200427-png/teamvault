...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { const r = await authApi.register(email, password, displayName); setAuth(r.user, r.access_token, r.refresh_token); navigate('/workspace') }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-bg-tertiary px-4 relative">
      <div className="bg-bg-secondary p-8 rounded-xl shadow-2xl w-full max-w-sm border border-border">
        <h1 className="text-2xl font-bold text-center text-text-primary mb-1">Create an account</h1>
        <p className="text-center text-text-muted text-sm mb-6">Join TeamVault today</p>
        ...
        <p className="text-sm text-text-muted mt-4">Already have an account? <Link to="/login" className="text-text-link hover:underline">Log In</Link></p>
      </div>
